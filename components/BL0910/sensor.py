    ...
    cv.Schema(
        {
            ...
            cv.Optional("Current_7"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "current",
            unit_of_measurement = "A"  
            ),
            cv.Optional("Current_8"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "current",
            unit_of_measurement = "A"  
            ),
            cv.Optional("Current_9"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "current",
            unit_of_measurement = "A"  
            ),
            cv.Optional("Current_10"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "current",
            unit_of_measurement = "A"  
            ),
            ...
            cv.Optional("Power_7"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "power",
            unit_of_measurement = "W" 
            ),
            cv.Optional("Power_8"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "power",
            unit_of_measurement = "W" 
            ),
            cv.Optional("Power_9"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "power",
            unit_of_measurement = "W" 
            ),
            cv.Optional("Power_10"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "power",
            unit_of_measurement = "W" 
            ),
            ...
            cv.Optional("Energy_7"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "energy",
            state_class = "total",
            unit_of_measurement = "kWh" 
            ),
            cv.Optional("Energy_8"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "energy",
            state_class = "total",
            unit_of_measurement = "kWh" 
            ),
            cv.Optional("Energy_9"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "energy",
            state_class = "total",
            unit_of_measurement = "kWh" 
            ),
            cv.Optional("Energy_10"): sensor.sensor_schema(
            accuracy_decimals = 3,
            device_class = "energy",
            state_class = "total",
            unit_of_measurement = "kWh" 
...
async def to_code(config):
    ...
    if "Current_7" in config:
        sens = await sensor.new_sensor(config["Current_7"])
        cg.add(var.set_current_sensor_7(sens))
    if "Current_8" in config:
        sens = await sensor.new_sensor(config["Current_8"])
        cg.add(var.set_current_sensor_8(sens))
    if "Current_9" in config:
        sens = await sensor.new_sensor(config["Current_9"])
        cg.add(var.set_current_sensor_9(sens))
    if "Current_10" in config:
        sens = await sensor.new_sensor(config["Current_10"])
    ...
    if "Power_7" in config:
        sens = await sensor.new_sensor(config["Power_7"])
        cg.add(var.set_power_sensor_7(sens))
    if "Power_8" in config:
        sens = await sensor.new_sensor(config["Power_8"])
        cg.add(var.set_power_sensor_8(sens))
    if "Power_9" in config:
        sens = await sensor.new_sensor(config["Power_9"])
        cg.add(var.set_power_sensor_9(sens))
    if "Power_10" in config:
        sens = await sensor.new_sensor(config["Power_10"])
    ...
    if "Energy_7" in config:
        sens = await sensor.new_sensor(config["Energy_7"])
        cg.add(var.set_energy_sensor_7(sens)) 
    if "Energy_8" in config:
        sens = await sensor.new_sensor(config["Energy_8"])
        cg.add(var.set_energy_sensor_8(sens)) 
    if "Energy_9" in config:
        sens = await sensor.new_sensor(config["Energy_9"])
        cg.add(var.set_energy_sensor_9(sens)) 
    if "Energy_10" in config:
        sens = await sensor.new_sensor(config["Energy_10"])
